...
export function candidatePresetIds(room:number,tier:AdaptiveTier,profile:PlayerPerformanceProfile|null,history:PreparedRoom[]):string[]{
 if(room<=4)return [["test_aim"],["test_priority"],["test_resource"],["test_movement"]][room-1];
 const boss=room%5===0;if(boss)return tierIndex(tier)>=3?["warden_rift","warden_approach"]:["warden_approach"];
...
export function materializeRoom(room:number,presetId:string,tier:AdaptiveTier,source:RoomSource="local",selection?:Partial<RoomPlannerSelection>):PreparedRoom{
 ...
 return{schemaVersion:1,roomSequenceIndex:room,source,presetId:preset.id,templateId:template.id,layoutId:template.layoutId,targetDifficultyScore:clamp(preset.difficultyScore,budget.min-8,budget.max+8),pacingRole:preset.pacingRole,spawnPattern:pattern,waves:waveGroups(preset,template,pattern,countMultiplier,aggression,reaction),hazardSetId:preset.hazardSetId,pickupSetId:preset.pickupSetId,objectiveId:preset.objectiveId,predictedKillRate:clamp(Number(selection?.predictedKillRate??midpoint(preset.expectedKillRateRange)),0,1),predictedCompletionTimeSeconds:clamp(Number(selection?.predictedCompletionTimeSeconds??midpoint(preset.expectedCompletionTimeRange)),20,150),predictedDamageTakenRatio:clamp(Number(selection?.predictedDamageTakenRatio??(.22+tierIndex(tier)*.08)),.05,.8),adaptationReasonCodes:(selection?.adaptationReasonCodes||["local_fallback"]).filter(x=>typeof x==="string").slice(0,4),confidence:clamp(Number(selection?.confidence??.7),0,1),mapStyleId,mapStyleSource:source==="gpt"?"openai":"selected",styleReasonCodes:source==="gpt"?["openai_style_plan"]:["player_style_choice"],plannerRecommendation:source==="gpt"&&selection?selection as RoomPlannerSelection:undefined};
}
...
export function prepareRoomQueue(startRoom:number,count:number,tier:AdaptiveTier,profile:PlayerPerformanceProfile|null,history:PreparedRoom[],seed:string):PreparedRoom[]{
 const out:PreparedRoom[]=[];for(let room=startRoom;room<startRoom+count;room++){const candidates=candidatePresetIds(room,tier,profile,[...history,...out]);const rng=seededRandom(`${seed}:${room}:${tier}`);const preset=candidates[Math.floor(rng()*candidates.length)]||"gallery_relief";out.push(materializeRoom(room,preset,tier))}return out;
}

export function validatePreparedRoom(plan:PreparedRoom):boolean{
 const preset=presetById.get(plan.presetId),template=templateById.get(plan.templateId);if(!preset||!template||preset.templateId!==template.id||plan.layoutId!==template.layoutId||!getMapStyle(plan.mapStyleId)||!isMapStyleCompatible(plan.mapStyleId,template.id))return false;
 if(plan.roomSequenceIndex%5===0&&preset.pacingRole!=="boss"||plan.roomSequenceIndex%5!==0&&preset.pacingRole==="boss")return false;
