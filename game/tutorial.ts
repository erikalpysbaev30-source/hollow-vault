...
export function currentTutorialPrompt(level:number,m:EncounterMetrics,family:InputFamily):TutorialPrompt|null{
 ...
 if(level===3){if(m.abilityUses<1)return{id:"l3-dash",level,title:"Dash through danger",text:control(family,"Move and press Space to dash.","Move and press A to dash.","Move and tap DASH.")};if((m.resourcesCollected||0)<1)return{id:"l3-resource",level,title:"Collect resources",text:"Defeated enemies may drop health, energy, or coins. Move over one pickup."};if((m.supersActivated||0)<1)return{id:"l3-super",level,title:"Charge your Super",text:control(family,"Land successful hits, then press Q when Energy Nova is ready.","Land successful hits, then press R3 when Energy Nova is ready.","Land successful hits, then tap SUPER when it is ready.")};}
...
const demonstrated=(level:number,m:EncounterMetrics)=>level===1?m.movementMs>=700&&m.shotsFired>0&&m.shotsHit>0:level===2?m.kills>=2:level===3?m.abilityUses>0&&(m.resourcesCollected||0)>0&&(m.supersActivated||0)>0:level===4?Boolean(m.reinforcementUsed):level===5?m.kills>=5:true;
