...
export interface EncounterMetrics{
 ...
 superSuccessfulHits?:number;superChargeEarned?:number;supersCharged?:number;supersActivated?:number;superTargetsHit?:number;superDamageDealt?:number;superEliteTargetsHit?:number;superBossDamage?:number;superHeldReadyMs?:number;roomsCompletedWhileSuperReady?:number;
 averageCombatDistanceRatio?:number;explorationRatio?:number;stationaryCombatRatio?:number;rushFrequency?:number;retreatFrequency?:number;coverUsageRate?:number;flankAvoidanceRate?:number;supportEnemyPriorityRate?:number;nearestEnemyTargetRate?:number;healEarlyRate?:number;healLateRate?:number;dominantWeaponId?:string;dominantAbilityId?:string;bossPhaseReached?:number;dominantDamageSource?:string;
}
...
export function calculateProfile(samples:EncounterMetrics[]):PlayerPerformanceProfile{
 ...
 return{schemaVersion:2,completedTestLevels:samples.some(s=>s.room>=5&&s.completed),encounters:samples.length,accuracy,reactionTimeMs:reaction,averageKillTimeMs:killTime,damageTakenPerMinute:damagePerMinute,movementEfficiency:movement,abilityUsageEfficiency:ability,consistencyScore:consistency,overallSkillScore:score,confidenceScore:confidence,dimensions:dims,lastUpdatedAt:new Date().toISOString()};
}
...
export function selectAutoAimTarget(origin:{x:number;y:number},mobs:{x:number;y:number;hp:number}[],obstacles:{x:number;y:number;w:number;h:number}[]){let best:null|{x:number;y:number;angle:number;score:number}=null;for(const m of mobs){if(m.hp<=0||!hasLineOfSight(origin,m,obstacles))continue;const dist=Math.hypot(m.x-origin.x,m.y-origin.y);if(dist>620||best&&dist>=best.score)continue;best={x:m.x,y:m.y,angle:Math.atan2(m.y-origin.y,m.x-origin.x),score:dist}}return best}
