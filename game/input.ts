...
export interface TouchGameplayInput{
 moveX:number;moveY:number;aimX:number;aimY:number;aiming:boolean;fire:boolean;fireQueued:boolean;autoAimQueued:boolean;dashPressed:boolean;supportPressed:boolean;
}

export const emptyTouchInput=():TouchGameplayInput=>({moveX:0,moveY:0,aimX:0,aimY:0,aiming:false,fire:false,fireQueued:false,autoAimQueued:false,dashPressed:false,supportPressed:false});
...
export function resetMutableInput(input:{keys:Set<string>;mouse:{down:boolean};touch:TouchGameplayInput;reinforcementButtonDown?:boolean}){
 input.keys.clear();input.mouse.down=false;Object.assign(input.touch,emptyTouchInput());if("reinforcementButtonDown" in input)input.reinforcementButtonDown=false;
}
