...
export function MobileControls({visible,leftHanded,superReady,superPercent,onMove,onAim,onAttackRelease,onDash,onSupport,onSuper,onPause}:{visible:boolean;leftHanded:boolean;superReady:boolean;superPercent:number;onMove:VectorHandler;onAim:VectorHandler;onAttackRelease:VectorHandler;onDash:()=>void;onSupport:()=>void;onSuper:()=>void;onPause:()=>void}){
 if(!visible)return null;return <div className={`mobile-controls ${leftHanded?"left-handed":""}`} aria-label="Touch game controls">
  ...
  <button className="touch-action dash" onPointerDown={onDash}>DASH</button><button className="touch-action support" onPointerDown={onSupport}>SUPPORT</button><button className={`touch-action super ${superReady?"ready":""}`} disabled={!superReady} onPointerDown={onSuper} aria-label={superReady?"Activate Energy Nova Super":`Super ${superPercent}% charged`}><span>SUPER</span><b>{superReady?"READY":`${superPercent}%`}</b></button><button className="touch-pause" onClick={onPause} aria-label="Pause game">Ⅱ</button>
 </div>;
}
